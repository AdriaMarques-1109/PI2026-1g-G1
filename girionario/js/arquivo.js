...
// Renderiza a lista filtrada na tela
function renderizarCards() {
    const container = document.getElementById("cards-container");
    container.innerHTML = "";

    // Aplica a busca e o filtro por região sobre os cards sorteados
    const cardsFiltrados = cardsAtuais.filter(card => {
        const atendeRegiao = filtroRegiao === "todos" || card.classe === filtroRegiao;
        
        const termo = termoBusca.toLowerCase();
        const atendeBusca = card.titulo.toLowerCase().includes(termo) || 
                            card.texto.toLowerCase().includes(termo) ||
                            card.detalhes.toLowerCase().includes(termo);

        return atendeRegiao && atendeBusca;
    });

    if (cardsFiltrados.length === 0) {
        container.innerHTML = "<p class='sem-resultados'>Nenhum card encontrado.</p>";
        return;
    }
...
// Configura os botões de filtro regional
function configurarFiltros() {
    const botoes = document.querySelectorAll(".btn-filtro");
    
    botoes.forEach(botao => {
        botao.addEventListener("click", () => {
            const ativoAtual = document.querySelector(".btn-filtro.ativo");
            if (ativoAtual) ativoAtual.classList.remove("ativo");
            
            botao.classList.add("ativo");
            filtroRegiao = botao.getAttribute("data-classe");
...
// Configura o evento do campo de busca
...
window.onload = () => {
    // Sorteia os 6 cards ao carregar a página
    cardsAtuais = embaralhar([...bancoDeDadosCards]).slice(0, 6);
